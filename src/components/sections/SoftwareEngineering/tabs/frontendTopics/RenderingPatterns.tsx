	...
	return (
		<div>
			...
			<p className='mb-6 text-gray-600'>
				...
				SEO, infrastructure costs, and developer experience. Understanding the trade-offs is essential for frontend system design.
			</p>
    ...
    <ul>
      {users.map(user => (
        <li key={user.id}>{user.name}</li>
			...
			<p className='mb-4 text-gray-600'>
				SSG and ISR occupy opposite ends of the static-dynamic spectrum. SSG pre-builds all pages at build time for maximum speed.
				ISR extends SSG with background revalidation so content stays fresh without full rebuilds.
			</p>
			...
			<div className='mb-6 grid gap-4 md:grid-cols-2'>
				...
				<div className='rounded-lg border border-green-200 bg-green-50 p-4'>
					...
					<p className='text-sm text-green-700'>
						Combines SSG's speed with near-real-time content updates. Pages are statically generated but can be revalidated in
						the background after a configurable interval. When a request arrives after the revalidation window, the stale page
						is served immediately while a fresh version is generated for subsequent requests.
					</p>
...
function LikeButton({ postId }) {
  ...
  return <button onClick={() => setLiked(!liked)}>{liked ? 'Liked' : 'Like'}</button>;
			...
			<p className='mb-4 text-gray-600'>
				Hydration is the process of making server-rendered HTML interactive. The strategy you choose determines how much JavaScript
				ships to the client and how quickly the page becomes interactive. The industry is moving from full hydration toward more
			...
			<div className='mb-6 rounded-xl border border-gray-200 bg-gray-50 p-6'>
				<div className='space-y-3'>
					...
					<div className='flex items-start gap-3'>
						...
						<p className='text-sm text-gray-700'>
							<strong>Is the page data-heavy with parts that load at different speeds?</strong> → Streaming SSR with Suspense.
							Show the fast parts immediately; stream slower sections as they resolve. No more all-or-nothing loading states.
						</p>
					</div>
					...
					<div className='flex items-start gap-3'>
						...
						<p className='text-sm text-gray-700'>
							<strong>Do you want to minimize client-side JavaScript?</strong> → React Server Components combined with Islands
							architecture. Only ship JS for the interactive parts; everything else stays on the server.
						</p>
					</div>
			...
			<div className='rounded-xl border border-blue-200 bg-blue-50 p-6'>
				...
				<ul className='space-y-2 text-blue-700'>
					...
					<li className='flex items-start'>
						<span className='mr-2 text-blue-500'>•</span>
						SSG is the fastest for static content; ISR adds near-real-time updates without requiring full rebuilds, making it
						practical for content-heavy sites.
					</li>
