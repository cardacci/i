		...
		<>
			...
			{isModalOpen && (
				<div className='fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50' onClick={() => setIsModalOpen(false)}>
					<div className='relative w-[95vw] max-h-[90vh] p-4' onClick={(e) => e.stopPropagation()}>
						{title && (
						...
						{hasMultiplePhotos && (
							<div className='hidden'>
								{loadedImages.has(currentPhotoIndex) && (
									<>
										{/* Preload next image */}
										{currentPhotoIndex < allPhotos.length - 1 && <img alt='preload' src={allPhotos[currentPhotoIndex + 1]} />}
